...
class CliffWalkingEnv(Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        self.shape = (2, 3)
        self.start_state_index = np.ravel_multi_index((1, 0), self.shape)
    ...
    def _calculate_transition_prob(self, current, delta):
        ...
        if self._cliff[tuple(new_position)]:
            return [(1.0, self.start_state_index, -1, False)]
        ...
        return [(1.0, new_state, 0, is_terminated)]
        ...
        if self.near_cliff_img is None:
            near_cliff_imgs = [
                path.join(path.dirname(__file__),
                          "img/mountain_near-cliff1.png"),
                path.join(path.dirname(__file__),
                          "img/mountain_near-cliff2.png"),
            ]
            self.near_cliff_img = [
                pygame.transform.scale(pygame.image.load(f_name), self.cell_size)
