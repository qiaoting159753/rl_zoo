...
class MVE_SAC_all:
    ...
    def _train_policy(
        ...
        with torch.no_grad():
            ...
            # Q = r + yr + y^2 * (q(ns, as(ns)) - pi(as(ns)))
            ...
            _, all_mean_next, pred_next_states, _, _ = self.world_model.pred_next_states(next_states, curr_hori_action)

            accum_rewards, _, _ = self.world_model.pred_multiple_rewards(rewards, self.gamma,
            ...
            for i in range(accum_rewards.shape[0]):
                pred_tq1 = accum_rewards[i] + not_dones * (self.gamma ** 2) * pred_v1
                pred_tq2 = accum_rewards[i] + not_dones * (self.gamma ** 2) * pred_v2
                pred_tq3 = accum_rewards[i] + not_dones * (self.gamma ** 2) * pred_v3
                pred_tq4 = accum_rewards[i] + not_dones * (self.gamma ** 2) * pred_v4
                q_0.append(pred_tq1)
                q_0.append(pred_tq2)
                q_0.append(pred_tq3)
                q_0.append(pred_tq4)
            ...
            #########################################################################################
            next_states = pred_next_states
            curr_hori_action, curr_hori_log_pi, _ = self.actor_net(next_states)
            _, all_mean_next, pred_next_states, _, _ = self.world_model.pred_next_states(next_states, curr_hori_action)

            print(accum_rewards.shape)

            pred_rewards, _ = self.world_model.pred_multiple_rewards(accum_rewards, ratio=self.gamma * self.gamma,
                                                                     observation=next_states,
                                                                     action=curr_hori_action,
                                                                     next_observation=all_mean_next)
            # V = Q - alpha * logi
            ...
            #     print
            #     pred_reward_2 = pred_rewards[i]
            #     pred_tq1 = rewards + not_dones * (self.gamma ** 2) * pred_v1 + self.gamma * pred_reward
            #     pred_tq2 = rewards + not_dones * (self.gamma ** 2) * pred_v2 + self.gamma * pred_reward
            #     pred_tq3 = rewards + not_dones * (self.gamma ** 2) * pred_v3 + self.gamma * pred_reward
            #     pred_tq4 = rewards + not_dones * (self.gamma ** 2) * pred_v4 + self.gamma * pred_reward
            #     q_0.append(pred_tq1)
            #     q_0.append(pred_tq2)
            #     q_0.append(pred_tq3)
            #     q_0.append(pred_tq4)
            ...
            # curr_hori_next_obs = next_states.unsqueeze(dim=0)
            ...
            #     horizon_obs_list = []
            #     curr_hori_qs = []
            #     # For current horizon state.
            #     for state_index in range(curr_hori_next_obs.shape[0]):
            #         sub_curr_hori_next_obs = curr_hori_next_obs[state_index]
            #         # Optimal sampling
            #         curr_hori_action, curr_hori_log_pi, _ = self.actor_net(sub_curr_hori_next_obs)
            #         #   Predict next_states first   #
            #         temp_pred_obs, all_mean_next, _, _ = self.world_model.pred_next_states(sub_curr_hori_next_obs,
            #                                                                                curr_hori_action)
            #         # Add all model's prediction to the list for future prediction.
            #         horizon_obs_list.append(all_mean_next)
            #
            #         # V = Q - alpha * logi
            #         pred_q1, pred_q2 = self.target_critic_net(sub_curr_hori_next_obs, curr_hori_action)
            #         pred_q3, pred_q4 = self.critic_net(sub_curr_hori_next_obs, curr_hori_action)
            #         pred_v1 = pred_q1 - self._alpha * curr_hori_log_pi
            #         pred_v2 = pred_q2 - self._alpha * curr_hori_log_pi
            #         pred_v3 = pred_q3 - self._alpha * curr_hori_log_pi
            #         pred_v4 = pred_q4 - self._alpha * curr_hori_log_pi
            #
            #         assert len(sub_curr_hori_next_obs.shape) == len(curr_hori_action.shape) == len(temp_pred_obs.shape)
            #         assert sub_curr_hori_next_obs.shape[0] == curr_hori_action.shape[0] == temp_pred_obs.shape[0]
            #
            #         pred_rewards, _ = self.world_model.pred_multiple_rewards(observation=sub_curr_hori_next_obs,
            #                                                                  action=curr_hori_action,
            #                                                                  next_observation=all_mean_next)
            #
            #         temp_disc_rewards = []
            #         # For each predict reward. 25 times of the input state.
            #         for rwd_index in range(pred_rewards.shape[0]):
            #             disc_pred_reward = not_dones * (self.gamma ** (hori + 1)) * pred_rewards[rwd_index]
            #
            #             if hori > 0:
            #                 for k in range(pred_all_next_rewards.shape[0]):
            #                     # Horizon = 1, 2, 3, 4, 5
            #                     disc_sum_reward = pred_all_next_rewards[k] + disc_pred_reward
            #                     temp_disc_rewards.append(disc_sum_reward)
            #                     assert rewards.shape == not_dones.shape == disc_sum_reward.shape == pred_v1.shape
            ...
            #                     curr_hori_qs.append(pred_tq1)
            #                     curr_hori_qs.append(pred_tq2)
            #                     curr_hori_qs.append(pred_tq3)
            #                     curr_hori_qs.append(pred_tq4)
            #             else:
            #                 # Horizon = 0
            #                 disc_sum_reward = disc_pred_reward
            #                 temp_disc_rewards.append(disc_sum_reward)
            ...
            # # Horizon level.
            # curr_hori_next_obs = torch.vstack(horizon_obs_list)
    ...
    def train_world_model(
        ...
        # print(F.mse_loss(next_s, next_states))
        # rwd_mean, _, _ = self.world_model.pred_rewards(states, actions, next_states)
        # print(F.mse_loss(rwd_mean, rewards))
