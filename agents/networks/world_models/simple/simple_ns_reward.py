...
from utils import weight_init, MLP
...
class Simple_NS_Reward(nn.Module):
    def __init__(self, observation_size: int, num_actions: int, hidden_size: list, normalize:bool):
        ...
        self.model = MLP(input_size=observation_size, hidden_sizes=hidden_size, output_size=1)
        self.add_module('mlp', self.model)
        self.model.apply(weight_init)

    def forward(
        self, observation: torch.Tensor) -> torch.Tensor:
        """
