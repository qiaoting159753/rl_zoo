    ...
    def evaluate(self):
        ...
        for _ in range(self.num_eval):
            ...
            for _ in range(self.max_epi_steps):
                ...
                model_error = math.sqrt(np.mean((pred - next_state) ** 2))

                tensor_next_state = torch.FloatTensor(next_state).to(self.device).unsqueeze(dim=0)
                pred_rwd = self.agent.world_model.pred_rewards(tensor_next_state)
                pred_rwd = pred_rwd.detach().cpu().item()
                reward_error = abs(pred_rwd - reward)
        ...
        avg_rewards = total_rewards / counter
